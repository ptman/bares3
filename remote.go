...
func sha256SignedPayload(body io.Reader, size int64) (string, io.ReadCloser, error) {
	if size < 0 {
		return "", nil, errors.New("content length must not be negative")
	}
	if body == nil {
		body = strings.NewReader("")
	}
	var buf bytes.Buffer
	if _, err := io.CopyN(&buf, body, size); err != nil {
		return "", nil, err
	}
	var extra [1]byte
	if n, err := body.Read(extra[:]); err != nil && err != io.EOF {
		return "", nil, err
	} else if n != 0 {
		return "", nil, errors.New("content length does not match body")
	}
	return HashSHA256(buf.Bytes()), io.NopCloser(bytes.NewReader(buf.Bytes())), nil
}
