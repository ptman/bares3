...
func TestCanonicalQueryString(t *testing.T) {
	...
	tests := []struct {
		...
		// Sort by name, not by the whole "name=value" pair: "a" is a prefix of
		// "a%21", and '%' (0x25) < '=' (0x3D) would misorder the old approach.
		{"a%21=1&a=2", "a=2&a%21=1"},
		// Identical names sort by value.
		{"x=b&x=a", "x=a&x=b"},
	...
	for _, tt := range tests {
		got := CanonicalQueryString(tt.input)
		if got != tt.expected {
			t.Errorf("CanonicalQueryString(%q) = %q, want %q", tt.input, got, tt.expected)
		}
	}
}
...
func TestSplitScheme(t *testing.T) {
	t.Parallel()
	tests := []struct {
		input, wantScheme, wantHost string
	}{
		{"https://s3.amazonaws.com", "https://", "s3.amazonaws.com"},
		{"http://minio.local", "http://", "minio.local"},
		{"s3.amazonaws.com", "http://", "s3.amazonaws.com"},
		{"", "http://", ""},
	}
	for _, tt := range tests {
		scheme, host := splitScheme(tt.input)
		if scheme != tt.wantScheme {
			t.Errorf("splitScheme(%q) scheme = %q, want %q", tt.input, scheme, tt.wantScheme)
		}
		if host != tt.wantHost {
			t.Errorf("splitScheme(%q) host = %q, want %q", tt.input, host, tt.wantHost)
		}
	}
}
...
func TestSha256SignedPayload(t *testing.T) {
	t.Parallel()
	body := []byte("hello world test data")
	hash, newBody, err := sha256SignedPayload(bytes.NewReader(body), int64(len(body)))
	if err != nil {
		t.Fatalf("sha256SignedPayload: %v", err)
	}
	expected := HashSHA256(body)
	if hash != expected {
...
func TestSha256SignedPayload_Empty(t *testing.T) {
	t.Parallel()
	body := []byte("")
	hash, newBody, err := sha256SignedPayload(bytes.NewReader(body), 0)
	if err != nil {
		t.Fatalf("sha256SignedPayload empty: %v", err)
	}
	if hash != HashSHA256([]byte("")) {
		t.Errorf("hash = %q, want %q", hash, HashSHA256([]byte("")))
	}
	data, err := io.ReadAll(newBody)
	if err != nil {
		t.Fatalf("ReadAll: %v", err)
...
func TestSha256SignedPayload_Truncation(t *testing.T) {
	t.Parallel()
	body := []byte("this is longer than the size we ask for")
	_, _, err := sha256SignedPayload(bytes.NewReader(body), 5)
	if err == nil {
		t.Fatal("sha256SignedPayload should reject a body longer than ContentLength")
	}
}
...
func TestSigV4CanonicalURIAndQueryBoundaries(t *testing.T) {
	...
	if got := CanonicalQueryString("a=%2F&a=/&a=&b"); got != "a=&a=%2F&a=/&b" {
		t.Errorf("CanonicalQueryString = %q", got)
	}
}

func TestSha256SignedPayloadReaderFailures(t *testing.T) {
	t.Parallel()
	for _, test := range []struct {
		...
		{"negative", strings.NewReader("x"), -1},
		{"short", strings.NewReader("x"), 2},
		{"reader error", failingReader{}, 1},
	} {
		t.Run(test.name, func(t *testing.T) {
			if _, _, err := sha256SignedPayload(test.body, test.size); err == nil {
				t.Fatal("expected error")
			}
		})
	}
	body := oneReadEOFReader{data: []byte("x")}
	hash, replay, err := sha256SignedPayload(&body, 1)
	if err != nil || hash != HashSHA256([]byte("x")) {
		t.Fatalf("one-read EOF = %q, %v", hash, err)
	}
...
type oneReadEOFReader struct{ data []byte }

func (r *oneReadEOFReader) Read(p []byte) (int, error) {
	if len(r.data) == 0 {
		return 0, io.EOF
	}
	p[0] = r.data[0]
