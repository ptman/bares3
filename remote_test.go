...
func TestSha256SignedPayload_Errors(t *testing.T) {
	t.Parallel()
	if _, _, err := sha256SignedPayload(strings.NewReader("x"), -1); err == nil {
		t.Error("negative size should fail")
	}
	if _, _, err := sha256SignedPayload(strings.NewReader("x"), 2); err == nil {
