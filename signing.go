...
// CanonicalQueryString builds the AWS SigV4 canonical query string. Input is
// an HTTP query string; parameters are decoded, AWS URI-encoded, then sorted
// by encoded name and encoded value.
func CanonicalQueryString(query string) string {
	...
	for pair := range strings.SplitSeq(query, "&") {
		...
		params = append(params, param{name, value, hasEquals})
	}
